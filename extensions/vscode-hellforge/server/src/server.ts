...
function findBridgePy(): string | undefined {
 ...
 */
let bridgeProc: import("child_process").ChildProcessWithoutNullStreams | undefined;
let bridgeReqId = 0;
const bridgePending = new Map<number, (v: any) => void>();
let bridgeBuf = "";
...
function startBridge(): boolean {
  ...
  proc.on("exit", () => {
    ...
    for (const cb of bridgePending.values()) cb(null);
    bridgePending.clear();
  });
  return true;
}
...
connection.onInitialized(() => {
  ...
  void callBridge("get_runtime_config", {}).then((cfg: any) => {
...
function lspSeverity(s: number): number {
  if (s === 0 || s === 1) return 1; // fatal/error → Error
  if (s === 2) return 2;            // warning → Warning
  if (s === 3) return 3;            // info → Information
  return 2;
}
...
function scheduleDiagnostics(uri: string) {
  ...
  diagTimers.set(
    ...
    setTimeout(() => {
      ...
      callBridge("get_diagnostics", { text }).then((diags: any[]) => {
        if (!diags) return;
        const mapped: Diagnostic[] = diags.map((d) => ({
          ...
          message: d.message,
          severity: lspSeverity(d.severity) as any,
          source: "hellforge",
        }));
...
connection.onRequest("hellforge/bridgeRequest", async (params: any) => {
  const { method, params: p } = params || {};
  if (!method) return { ok: false, error: "no method" };
