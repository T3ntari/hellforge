 ...
 */
function findHellforgeRoot() {
    // 1. User settings override
    const cfg = vscode_1.workspace.getConfiguration("hellforge");
    const configured = cfg.get("projectPath");
    if (configured) {
        const p = expandHome(configured);
        if (isHellforgeRoot(p))
            return p;
    }
    // 2. Environment variable
    const envHome = process.env.HELLFORGE_HOME;
    if (envHome && isHellforgeRoot(envHome))
        return envHome;
    // 3. Active editor file — walk up from the open document
    const active = vscode_1.window.activeTextEditor?.document.uri.fsPath;
    if (active) {
        let dir = path.dirname(active);
        for (let i = 0; i < 12; i++) {
            if (isHellforgeRoot(dir))
                return dir;
...
function runHellforge(args, inWindow = false, cwd) {
    ...
    if (!root) {
        ...
        vscode_1.window.showErrorMessage(`HELLFORGE not found. Searched: ${searched}. ` +
    ...
    const env = { ...process.env, HELLFORGE_PROJECT: root };
    const term = vscode_1.window.createTerminal({ name, cwd: root, env });
    term.sendText(cmd);
    term.show();
}
...
function openShell() {
    const cfg = vscode_1.workspace.getConfiguration("hellforge");
    const shellType = cfg.get("shellType") || "eshell";
    if (shellType === "hshell") {
        ...
        if (!root) {
            vscode_1.window.showErrorMessage("HELLFORGE not found. Set hellforge.projectPath in settings or open a workspace with ep.py.");
            return;
        }
        const env = { ...process.env, HELLFORGE_PROJECT: root };
        const term = vscode_1.window.createTerminal({
            name: "HELLFORGE",
            cwd: root,
            env,
            shellPath: pyCmd(),
...
function activate(context) {
    ...
    const status = vscode_1.window.createStatusBarItem(vscode_1.StatusBarAlignment.Right, 100);
    status.text = "$(terminal) HELLFORGE";
    status.tooltip = "HELLFORGE: Open Shell / Settings / Convert";
    status.command = "hellforge.menu";
    status.show();
    context.subscriptions.push(status);
    const gear = vscode_1.window.createStatusBarItem(vscode_1.StatusBarAlignment.Right, 99);
    gear.text = "$(gear)";
    ...
    let bridgeRequester = async () => null;
    const bridgeRequestCommand = vscode_1.commands.registerCommand("hellforge.bridgeRequest", (method, params) => bridgeRequester(method, params));
    context.subscriptions.push(bridgeRequestCommand);
    context.subscriptions.push(vscode_1.commands.registerCommand("hellforge.play", async () => {
        let f = activeFile();
        ...
        if (f)
            runHellforge(["play", f]);
    }), vscode_1.commands.registerCommand("hellforge.playWindow", async () => {
        let f = activeFile();
        if (!f || !/\.(e|ei|eic|enx|eci|machine|human|mid|wav|mp3|mp4)$/i.test(f)) {
            const picked = await vscode_1.window.showOpenDialog({
                canSelectMany: false,
        ...
        if (f)
            runHellforge(["play", f, "--gui"], true);
    }), vscode_1.commands.registerCommand("hellforge.openShell", () => {
        openShell();
    }), vscode_1.commands.registerCommand("hellforge.selectShell", async () => {
        ...
        if (pick) {
            await vscode_1.workspace.getConfiguration("hellforge").update("shellType", pick.value, true);
            openShell();
        }
    }), vscode_1.commands.registerCommand("hellforge.settings", () => {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const { SettingsPanel } = require("./settingsPanel");
        SettingsPanel.createOrShow(context.extensionUri);
    }), vscode_1.commands.registerCommand("hellforge.menu", async () => {
        const pick = await vscode_1.window.showQuickPick([
        ...
        if (!pick)
            return;
        if (pick.value === "shell")
            await vscode_1.commands.executeCommand("hellforge.openShell");
        else if (pick.value === "settings")
            await vscode_1.commands.executeCommand("hellforge.settings");
        else if (pick.value === "convert")
            await vscode_1.commands.executeCommand("hellforge.convert");
        else if (pick.value === "play")
            await vscode_1.commands.executeCommand("hellforge.play");
    }), vscode_1.commands.registerCommand("hellforge.convert", () => {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const { convertActiveFile } = require("./convert");
        void convertActiveFile(bridgeRequester);
    }));
    // Auto-compile on save (opt-in setting)
    const saveDisposable = vscode_1.workspace.onDidSaveTextDocument((doc) => {
        const cfg = vscode_1.workspace.getConfiguration("hellforge");
        if (!cfg.get("autoCompileOnSave"))
            return;
        if (!/\.(e|ei|eic|enx|eci)$/i.test(doc.fileName))
            return;
    ...
    if (!fs.existsSync(serverModule)) {
        vscode_1.window.showErrorMessage(`HELLFORGE LSP server missing: ${serverModule}. Reinstall the extension.`);
        return;
    }
    const serverOptions = {
    ...
    const clientOptions = {
        ...
        synchronize: { fileEvents: vscode_1.workspace.createFileSystemWatcher("**/*.{e,ei,eic,enx,eci,machine,human}") },
    };
    client = new node_1.LanguageClient("hellforgeLanguage", "HELLFORGE E Language", serverOptions, clientOptions);
    client.start();
    // Surface bridge failures (lint/completions disabled) instead of silence
    client.onNotification("hellforge/bridgeDown", () => {
        vscode_1.window.showErrorMessage("HELLFORGE bridge failed to start — linting and completions are disabled. " +
    ...
    bridgeRequester = (method, params) => client
        ? client.sendRequest("hellforge/bridgeRequest", { method, params })
