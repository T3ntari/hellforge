...
class SettingsPanel {
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel("hellforgeSettings", "HELLFORGE Settings", column, { enableScripts: true, retainContextWhenHidden: true });
    ...
    sendRequest(method, params) {
        return new Promise((resolve) => {
            ...
            void handler("hellforge.bridgeRequest", method, params).then((result) => resolve(result), () => resolve(null));
        });
    }
    ...
    async onMessage(msg) {
        switch (msg?.type) {
            ...
            case "apply": {
                ...
                await ext.update("autoCompileOnSave", !!(msg.advanced && msg.advanced.auto_compile_on_save), vscode.ConfigurationTarget.Global);
                await ext.update("playWindowed", !!(msg.advanced && msg.advanced.play_windowed), vscode.ConfigurationTarget.Global);
                vscode.window.showInformationMessage(applied ? "HELLFORGE settings applied (memory, threads, GPU acceleration are live)." : "Settings applied.");
                if (applied)
                    this.advanced = applied;
                break;
            }
            case "cancel": {
