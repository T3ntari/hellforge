...
def list_installed(pkg_type):
    ...
    for name, path in _find_packages(target):
        ...
        # Verification badge (import from fentclient.security if available)
        try:
...
def fetch(name, pkg_type):
    ...
    if result:
        # Post-install: verify with fentclient if present
        _verify_with_fentclient(name, target)
        _auto_reload()
    return result


def _verify_with_fentclient(name, target_dir):
    """If fentclient security module is available, verify the installed plugin."""
    try:
        mod = __import__("plugins.fentclient.security", fromlist=[""])
        if hasattr(mod, "compute_plugin_hash") and hasattr(mod, "load_pkglist_verifications"):
            h = mod.compute_plugin_hash(name=name)
            if h:
                codes = mod.load_pkglist_verifications()
                expected = codes.get(name, "")
                if expected:
                    if h == expected:
        ...
        pass  # fentclient not installed — no verification
    except Exception:
        pass
