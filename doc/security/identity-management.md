...
Authenticates with the oshonet.in server using your signing key. Creates a session token for subsequent server interactions.
...
Sessions are cached tokens that allow authenticated communication with the oshonet.in backend without re-entering credentials.
