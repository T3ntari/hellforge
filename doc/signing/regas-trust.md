...
REGAS is the highest trust level in the HELLFORGE signing hierarchy. A REGAS signature indicates the plugin author's identity has been confirmed by the oshonet.in server backend.
...
2. Key fingerprint is submitted to `/e_identity/submit`
3. Server reviews and confirms via `/e_identity/review`
