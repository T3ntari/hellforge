# HellGate — the OpenCode Wrapper

**Navigation:** [doc/index.md](../index.md) | [overview](overview.md) | [hellgate](hellgate.md) | [README](../../plugins/hellgate/README.md)

---

## Overview

**HellGate** (v0.1.14.41 — the HELLGATE_VERSION) is a wrapper that boots
and launches **OpenCode** directly, focused inside the HELLFORGE root, with
the full E Language knowledge pack fed in.

```bash
run.py hellgate        # or: krip hellgate (inside the sandbox)
```
...
2. **First-run onboarding** on a NEW machine — detected by machine specs
   (cpu/arch/hostname/python), not session history, so a fresh PC with old
   state still gets the questions:
...
4. **HellCode welcome page** — banner + `HellGate v<version>`,
   `- T3ntari` tiny at the bottom, and a loading screen: dots spinner,
   real init steps (imports, compile checks, self-tests, knowledge pack,
   config) with an **x/1024 counter** below the loader.
...
- **Confinement** — OpenCode runs with cwd = the project dir and all
  configs land under `hellgate-state/` inside the repo; no global config
  writes.
- **Knowledge** — `knowledge/full.md` (comprehensive, v5-first),
  `core.md` (distilled ~30% digest), `samples-index.md`, `agents.md`
...
- **Low-context summarizer** — `summarizer.py` re-digests the docs with
  the local Qwen2.5-Coder-3B (ollama) into `knowledge/core-llm.md`.
- **Providers** — `hellgate-state/provider.json` drives every launch.
  ...
  ollama path. All opt-in — nothing is hardcoded, no credentials ship.
...
See also: [LLM plugin](llm.md) · [LLM commands](../commands/llm-commands.md)
