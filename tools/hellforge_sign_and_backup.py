...
savidentity_public_key("REGAS", REGAS_PUB)
...
from plugins.verify.security import compute_plugin_hash
...
plugin_descriptions = {
    ...
    "verify": "Performance accelerator, bug fixes, enhanced syntax, FluidSynth, arpeggiator, cache",
...
plugin_tags = {
    ...
    "verify": TAGS + ",performance,acceleration,bugfix,soundfont",
    ...
    if name in hashes:
        if name in pkglist.get("plugins", {}):
            ...
            info = {
                ...
                "verification_domain": "registry.example.invalid",
            }
...
# ── Phase 4: Generate verify.json ──

print(f"\nPhase 4: Generating verify.json...\n")

everify = {}
for name, h in hashes.items():
    everify[name] = h

everify_path = os.path.join(PROJECT, "verify.json")
with open(everify_path, "w") as f:
    json.dump(everify, f, indent=2)
print(f"  -> verify.json saved ({len(everify)} entries)")
